...
export function initJavaGenerator() {
  ...
  g.forBlock['program_main'] = function (
    block: Blockly.Block,
...
`;
  ...
  g.forBlock['var_declare'] = function (block, gen) {
    ...
    (gen as any).definitions_[`var_${name}`] = `${t} ${name};`;
    return '';
  };
  ...
  g.forBlock['var_set_custom'] = function (block, gen) {
    const name = getVarName(gen, block, 'VAR');
    const value =
      gen.valueToCode(block, 'VALUE', (gen as any).ORDER_NONE) || '0';
  ...
  g.forBlock['io_read'] = function (block, gen) {
    ...
    if (t === 'double')
      return ['scanner.nextDouble()', (gen as any).ORDER_ATOMIC];
    if (t === 'boolean')
      return ['scanner.nextBoolean()', (gen as any).ORDER_ATOMIC];
  ...
  g.forBlock['ctrl_if'] = function (block, gen) {
    const cond =
      gen.valueToCode(block, 'COND', (gen as any).ORDER_NONE) || 'false';
  ...
  g.forBlock['ctrl_for'] = function (block, gen) {
    ...
    const stepRaw =
      gen.valueToCode(block, 'STEP', (gen as any).ORDER_NONE) || '1';
    const stepLiteral = parsePositiveIntegerLiteral(stepRaw);
    const stepExpr =
      stepLiteral !== null
        ? String(stepLiteral)
        : `(int)Math.max(1, Math.abs(${stepRaw}))`;
  ...
  g.forBlock['array_declare_1d'] = function (block, gen) {
    ...
    (gen as any).definitions_[`arr_${name}`] =
      `${t}[] ${name} = new ${t}[${len}]; // base=${base}`;
    return '';
  };
  ...
  g.forBlock['array_declare_2d'] = function (block, gen) {
    ...
    (gen as any).definitions_[`arr_${name}`] =
      `${t}[][] ${name} = new ${t}[${rows}][${cols}]; // base1=${base1}, base2=${base2}`;
    return '';
  };
  ...
  g.forBlock['text'] = function (block) {
    ...
    return [
      `"${escapeJavaString(String(t))}"`,
      (ensureGenerator() as any).ORDER_ATOMIC,
    ];
  };
  ...
  g.forBlock['text_join'] = function (block, gen) {
    ...
    for (let i = 0; i < itemCount; i += 1) {
      parts.push(
        gen.valueToCode(block, `ADD${i}`, (gen as any).ORDER_NONE) || '""',
      );
    }
  ...
  g.forBlock['math_randi'] = function (block, gen) {
    const from = gen.valueToCode(block, 'FROM', (gen as any).ORDER_NONE) || '0';
    const to = gen.valueToCode(block, 'TO', (gen as any).ORDER_NONE) || '0';
    (gen as any).definitions_[`random_decl`] =
  ...
  g.forBlock['text_upper'] = function (block, gen) {
    const text =
      gen.valueToCode(block, 'TEXT', (gen as any).ORDER_NONE) || '""';
  ...
  g.forBlock['math_arithmetic'] = function (block, gen) {
    ...
    if (op === 'POWER')
      return [`Math.pow(${A}, ${B})`, (gen as any).ORDER_ATOMIC];
    const sym = map[op] || '+';
    const order =
  ...
  g.forBlock['logic_operation'] = function (block, gen) {
    ...
    const order =
      op === 'OR'
        ? (gen as any).ORDER_LOGICAL_OR
        : (gen as any).ORDER_LOGICAL_AND;
    return [`(${A} ${sym} ${B})`, order];
  };

  g.forBlock['logic_negate'] = function (block, gen) {
    const A =
      gen.valueToCode(block, 'BOOL', (gen as any).ORDER_NONE) || 'false';
...
export function generateJavaFromWorkspace(workspace: Blockly.Workspace) {
  ...
  if (!program)
    return `// Dica: adicione o bloco "programa" (categoria Programa) para gerar a classe Java.\n`;
...
function indent(code: string, spaces = 2) {
  ...
  return code
    .split('\n')
    .filter(
      (_, idx, arr) => !(idx === arr.length - 1 && arr[idx].trim() === ''),
    )
