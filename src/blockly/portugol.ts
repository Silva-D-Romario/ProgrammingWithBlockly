...
function ensureGenerator() {
  ...
  g.forBlock['ctrl_if'] = function (block, gen) {
    const cond =
      gen.valueToCode(block, 'COND', (gen as any).ORDER_NONE) || 'falso';
  ...
  g.forBlock['ctrl_for'] = function (block, gen) {
    ...
    const stepExpr =
      stepLiteral !== null ? String(stepLiteral) : `abs(${stepRaw})`;
  ...
  g.forBlock['array_declare_2d'] = function (block, gen) {
    ...
    (gen as any).definitions_[`arr_${name}`] =
      `${name}: vetor[${base1}..${end1},${base2}..${end2}] de ${t}`;
    return '';
  };
  ...
  g.forBlock['math_arithmetic'] = function (block, gen) {
    ...
    if (op === 'MOD')
      return [`(${a} mod ${b})`, (gen as any).ORDER_MULTIPLICATIVE];
  ...
  g.forBlock['logic_negate'] = function (block, gen) {
    const a =
      gen.valueToCode(block, 'BOOL', (gen as any).ORDER_NONE) || 'falso';
...
export function generatePortugolFromWorkspace(workspace: Blockly.Workspace) {
  ...
  if (!program)
    return `// Dica: adicione o bloco "programa" para gerar o código em Portugol.\n`;
